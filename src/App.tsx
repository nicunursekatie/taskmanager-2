...
function App() {
  ...
  const [editTaskProjectId, setEditTaskProjectId] = useState<string | null>(
    null,
  );
  ...
  const isDateBefore = (taskDate: string, compareDate: Date): boolean => {
    ...
    const normalizedCompareDate = new Date(
  ...
  const overdueTasks = tasks.filter(
    (task) =>
      task.dueDate &&
      isDateBefore(task.dueDate, todayStart) &&
  ...
  const todayTasks = tasks.filter(
    (task) =>
      task.dueDate &&
      isDateBetween(task.dueDate, todayStart, tomorrowStart) &&
      task.status !== "completed",
  );
  ...
  // Define routes conditionally but use them inside the Router
  const tempoRoutes = import.meta.env.VITE_TEMPO === "true" ? routes : [];

  return (
    <Router>
      <div className="min-h-screen bg-background font-sans">
        {/* Tempo routes - always call useRoutes inside Router */}
        ...
        <header className="sticky top-0 z-30 shadow-md flex items-center justify-between px-8 py-4 border-b border-border">
          <h1 className="text-3xl font-extrabold text-primary tracking-tight relative after:absolute after:left-0 after:-bottom-1 after:w-16 after:h-1 after:bg-primary-light after:rounded-full after:content-['']">
            Task Manager
          </h1>
                  ...
                  <div className="space-y-8">
                    <div className="dashboard-grid">
                      ...
                      <div className="bg-white border border-border rounded-lg shadow-sm p-4 mb-6">
                        <h2 className="text-lg font-bold text-primary mb-2 border-b border-border pb-1">
                          Today's Tasks
                        </h2>
                        {todayTasks.length > 0 ? (
                          <TaskList
                          ...
                          <p className="text-text-light italic text-center py-4">
                            No tasks due today.
                          </p>
                        )}
                      </div>
                          ...
                          {todayTasks.length > 0 ? (
                            ...
                            <div className="empty-state">
                              ...
                              <div className="empty-state-description">
                                Great! You have a clear schedule for today.
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="bg-white border border-border rounded-lg shadow-sm p-4 mb-6">
                          ...
                          <div className="space-y-4">
                            {upcomingTasks.map((task) => (
                              <div
                                key={task.id}
                                className="flex items-center gap-3 py-2 px-3 border-b border-border last:border-b-0"
                              >
                      ...
                      <div className="bg-white border border-border rounded-lg shadow-sm p-4 mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {projects.map((project) => {
                            ...
                            // Calculate progress
                            const totalProjectTaskCount = tasks.filter(
                            ...
                            const tasksWithDueDates = projectTasks.filter(
                              (t) => t.dueDate,
                            );
                            const nearestDueDate =
                              tasksWithDueDates.length > 0
                                ? tasksWithDueDates.reduce(
                                    (nearest, task) =>
                                      !nearest.dueDate ||
                                      (task.dueDate &&
                                        task.dueDate < nearest.dueDate)
                                        ? task
                                        : nearest,
                                    tasksWithDueDates[0],
                                  )
                              ...
                              >
                                ...
                                {nearestDueDate && nearestDueDate.dueDate && (
                                  <div className="text-sm text-text-light">
                                    ...
                                    <p className="mt-1 text-text truncate">
                                      {nearestDueDate.title}
                                    </p>
                                  </div>
                                )}
                                ...
                                <div className="mt-4 space-y-2">
                                  {projectTasks.slice(0, 3).map((task) => (
                                    <div
                                      key={task.id}
                                      className="flex items-center gap-2 p-2 rounded-md hover:bg-primary/5 transition"
                                    >
                                      <input
                                        type="checkbox"
                                        checked={false}
                                      ...
                                      >
                                        ...
                                        {task.dueDate && (
                                          <span
                                            className={`text-xs ${isDateBefore(task.dueDate, todayStart) ? "text-danger" : "text-text-light"}`}
                                          >
                                            {new Date(
                                              task.dueDate,
                                            ).toLocaleDateString("en-US", {
                        ...
                        projects.map((project) => {
                          ...
                          return (
                            <div
                              ...
                              key={project.id}
                              className="project-card flex flex-col gap-md p-lg mb-md shadow-md border border-border rounded-xl bg-white"
                            >
                              <div className="flex items-start justify-between mb-2">
                                <div>
                                  <h3 className="text-lg font-bold text-primary mb-xs">
                                    {project.name}
                                  </h3>
                                  {project.description && (
                                    <p className="text-sm text-light mb-xs">
                                      {project.description}
                                    </p>
                                  )}
                                </div>
                        ...
                        {categories.map((category) => {
                          ...
                          return (
                            <div
                              ...
                              onClick={() => {
                                console.log(
                                  "Setting selected category ID to:",
                                  category.id,
                                );
                            ...
                            >
                              ...
                              <div className="compact-category-content">
                                <h3 className="compact-category-title">
                                  {category.name}
                                </h3>
                                ...
                                {recentTasks.length > 0 && (
                                  <div className="category-recent-task">
                                    <div className="recent-task-label">
                                      Recent task:
                                    </div>
                    ...
                    <div className="single-category-view">
                      <div className="view-header with-back-button">
                        <button
                          ...
                          onClick={() => {
                            console.log(
                              "Setting selected category ID back to null",
                            );
                            setSelectedCategoryId(null);
                          }}
                        >
                      ...
                      <div
                        ...
                        style={{
                          backgroundColor: `${categories.find((c) => c.id === selectedCategoryId)?.color}10`,
                        }}
                      >
                            ...
                            <form
                              onSubmit={(e) => {
                                ...
                                if (input && input.value.trim()) {
                                  addTask(input.value.trim(), null, undefined, [
                                    selectedCategoryId,
                                  ]);
                                  input.value = "";
                                }
                          ...
                          <span className="task-counter">
                            {
                              tasks.filter(
                                (t) =>
                                  t.categories?.includes(selectedCategoryId) &&
                                  t.status !== "completed",
                              ).length
                            }
                          </span>
                              ...
                              .map((project) => {
                                ...
                                return (
                                  <div
                                    ...
                                    style={{
                                      borderLeft: `4px solid ${project.color || categories.find((c) => c.id === selectedCategoryId)?.color}`,
                                    }}
                                    onClick={() => {
                                      setActiveTab("projects");
                                      setTimeout(() => {
                      ...
                      {tasks.filter(
                        ...
                        <div className="category-detail-section">
                          <div className="section-header">
                            ...
                            <span className="task-counter completed">
                              {
                                tasks.filter(
                                  (t) =>
                                    t.categories?.includes(
        ...
        <ReminderSystem
          ...
          openTask={(taskId) => {
            ...
            setEditTaskDueDate(
              tasks.find((t) => t.id === taskId)?.dueDate || "",
            );
            setEditTaskDueTime(
              tasks.find((t) => t.id === taskId)?.dueTime || "",
            );
            setEditTaskCategories(
              tasks.find((t) => t.id === taskId)?.categories || [],
            );
            setEditTaskProjectId(
              tasks.find((t) => t.id === taskId)?.projectId ?? null,
            );
            setEditTaskPriority(
              tasks.find((t) => t.id === taskId)?.priority ?? null,
            );
            setShowTaskEditModal(true);
                ...
                <form className="flex flex-col gap-md">
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      <button
                        type="button"
                        className={`btn btn-sm btn-outline ${editTaskDueDate === new Date().toISOString().split("T")[0] ? "bg-primary/10 text-primary" : ""}`}
                        onClick={() =>
                          setEditTaskDueDate(
                    ...
                    <div className="flex flex-wrap gap-2">
                      {categories.map((category) => (
                        <button
                          ...
                          style={{
                            backgroundColor: editTaskCategories.includes(
                              category.id,
                            )
                              ? category.color
                              : "transparent",
                            color: editTaskCategories.includes(category.id)
                              ? "#fff"
                              : category.color,
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEditTaskProjectId(e.target.value || null)
                      }
                    >
                  ...
                  <div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {["critical", "high", "medium", "low", null].map(
                        (priority) => (
                          <button
                            type="button"
                            key={priority ?? "none"}
        ...
        <MoreOptionsMenu
          ...
          onLoadSample={() =>
            loadSampleData(setTasks, setCategories, setProjects)
          }
